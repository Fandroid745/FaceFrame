...
class FaceEmbedder(private val context: Context) : AutoCloseable {
    ...
    private fun initModel() {
        try {
            ...
            if (modelName != null) {
                ...
                Log.i(TAG, "Successfully loaded TFLite model '$modelName'. Output: $embeddingDim-d")
            } else {
                Log.w(TAG, "No face embedding model found in assets. Using fallback.")
                isUsingFallback = true
                embeddingDim = 128
            }
        } catch (e: Exception) {
            Log.e(TAG, "Failed to load TFLite model, using fallback", e)
            isUsingFallback = true
            embeddingDim = 128
        }
    }
    ...
    fun extractEmbedding(faceBitmap: Bitmap): FloatArray {
        ...
        return try {
            ...
            Log.e(TAG, "Error running TFLite face embedding", e)
    ...
    private fun generateFallbackEmbedding(faceBitmap: Bitmap): FloatArray {
        ...
        for (i in vector.indices) vector[i] -= mean
